...
| Related tickets | None named in `SRC-1`; implementation-control Linear issue or project pending as an entry condition. |
...
| SRC-4 | Temporal TypeScript SDK `1.16.0` from `package-lock.json` plus pinned Temporal reference snapshot `/Users/jasonbelmonti/.codex/plugins/cache/openai-curated/temporal/4ce8a86b/skills/temporal-developer/` files `references/core/determinism.md`, `references/core/patterns.md`, `references/typescript/patterns.md`, `references/typescript/testing.md`, and `references/typescript/data-handling.md` | Temporal implementation authority for this spec, pinned to local SDK package versions and reviewable reference files. | Requires deterministic workflow code, side effects in regular activities, typed Updates for state mutations that return results, read-only non-blocking Update validators, activity heartbeats for cancellation delivery, compact payload handling, visibility/search attributes or a durable projection for cross-run discovery, and replay/versioning controls for command-sequence changes. |
| SRC-5 | `agent-config-registry` repository at `/Users/jasonbelmonti/Documents/Development/agent-config-registry`, commit `b011664409b32967de9584e13c393b8b7ddd20f1` (`origin/main` on 2026-04-29), tracked files `docs/zero-dollar-agent-config-registry-design.md`, `docs/zero-dollar-agent-config-registry-execution.md`, `src/agent-config-registry/contracts/**`, `src/agent-config-registry/validation/**`, and `docs/evidence/zero-dollar-registry/**` | Registry source and dependency baseline. Uncommitted `.worktrees/**` entries in that checkout are not source authority for this spec. | Registry currently defines `skill`, `agent-instructions`, and sync `profile` package kinds plus catalog/digest contracts. The composable workflow effort must add workflow-specific records without confusing runtime `execution-profile` records with existing sync profiles. |
| SRC-6 | Repository operating manual and user project-management constraints | Local execution authority. | Work shall prefer repo-local git worktrees, and Linear tasks created for this effort shall include Objective, Context / Constraints, Materially verifiable success criteria, and Execution notes. |
...
| DEP-3 | Dependency | Registry contract, validation, and catalog work must be stable enough to extend before formal workflow record implementation starts. | Registry implementation agent | Yes before WP-2 | DP-3 records whether to wait for the active registry branch to stabilize or implement workflow records on a coordinated registry worktree. |
...
2. Execute WP-1 as the first proving slice. Any registry-adjacent work in WP-1 is workflow-local fixture-only, shall not edit `agent-config-registry`, and shall not decide package-kind, schema, or catalog strategy before Q-3 and DP-3.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| MS-1 | Approve fake multi-run queue proof. | OBJ-1, OBJ-3, OBJ-4, WP-1, PKG-3, PKG-4, PKG-5 | Before WP-2 or WP-3 expands implementation | Jason Belmonti | VAL-1, EVD-1, and EVD-0 confirmation that WP-1 changed no `agent-config-registry` contract, schema, validation, or catalog files | REV-1, REV-3 | EVD-1 and WP-1 registry-boundary note in EVD-0 | Approve / Reject / Conditional approval | Stop; revise recipe/gate model before broad implementation. If WP-1 needs registry contract/schema changes, stop WP-1 and move that work behind WP-2/MS-2 or add an explicit `DEV-*` with `REV-2` approval. |
| MS-2 | Approve registry workflow contracts. | OBJ-2, WP-2, PKG-1, PKG-2, SURF-1, SURF-2 | Before WP-4 and before real adapters reference execution profiles | Jason Belmonti | Named `REV-2` reviewer assignment recorded in EVD-0; VAL-2, VAL-4, VAL-16, EVD-2, EVD-4, EVD-16 | REV-2 | EVD-2, EVD-4, EVD-16 | Approve / Reject / Conditional approval | Stop registry integration; revise package-kind, schema strategy, or compatibility posture. |
...
| --- | --- | --- | --- | --- | --- |
| VAL-0 | Review / PM inspection | WP-1 entry readiness is recorded: `SRC-1` approval, spec approval or conditional approval for WP-1, Q-1 decision, Linear execution-control issue or project with required headings, WP-1 registry-boundary note confirming no `agent-config-registry` contract/schema/validation/catalog edits, and named reviewer assignments for each `REV-*` gate before its dependent milestone can be approved. Q-3 and DEP-3 are recorded before WP-2 starts. | Before WP-1 starts; update before WP-2 starts for Q-3 and DEP-3; update before any milestone whose dependent `REV-*` reviewer has not yet been named | Jason Belmonti and Codex implementation agent | EVD-0 |
...
`SRC-1` Q-1 maps to this spec's Q-2 queue storage decision. `SRC-1` D-1 constrains this spec's Q-4 consensus-review execution-shape decision. This spec's Q-1, Q-3, and Q-5 are execution-specific gates added during planning.
...
| SURF-7 / Validation test surface | SURF-7 | PKG-1, PKG-2, PKG-3, PKG-4, PKG-5, PKG-6, PKG-7 | WP-1, WP-2, WP-3, WP-4, WP-5, WP-6, WP-7 | Entry gate, MS-1, MS-2, MS-3, MS-4, MS-5 | CTRL-1, CTRL-6, CTRL-8, CTRL-9, CTRL-10 | VAL-0, VAL-1, VAL-2, VAL-3, VAL-4, VAL-5, VAL-6, VAL-7, VAL-8, VAL-9, VAL-10, VAL-11, VAL-12, VAL-13, VAL-14, VAL-15, VAL-16, VAL-17, VAL-18 | REV-1, REV-1A, REV-2, REV-3, REV-4, REV-5, REV-6 | REL-2, REL-3, REL-4, REL-5, OBS-7, OBS-8, OBS-9 | EVD-0, EVD-1, EVD-2, EVD-3, EVD-4, EVD-5, EVD-6, EVD-7, EVD-8, EVD-9, EVD-10, EVD-11, EVD-12, EVD-13, EVD-14, EVD-15, EVD-16, EVD-17, EVD-18 |
