...
The shortest path that proves the design is viable is: validate a minimal recipe and runtime execution profile, resolve it to an immutable snapshot, start a Temporal recipe run, execute a fake step, emit a queue item, reject a stale decision, accept the matching decision, and complete the run while preserving deterministic workflow boundaries.

First proving slice:
WP-1 shall implement a fake recipe-to-gate vertical slice with minimal registry fixture records, a pinned snapshot, a recipe-backed Temporal workflow path, one fake blocking step, a queue item query, stale-decision rejection, and matching approval resume. This slice shall use fake activities only and shall not adapt real skills or live providers.
...
| --- | --- | --- | --- | --- |
| MV-1 | MS-1 | Start the fake recipe run and query state while blocked. | Query shows pinned recipe snapshot, active step, open queue item ID, gate revision, and fake artifact refs. | EVD-1 |
| MV-2 | MS-1 | Submit a stale or mismatched decision signal. | Workflow rejects the decision and remains blocked on the current queue item. | EVD-1 |
| MV-3 | MS-1 | Submit the matching approval decision. | Workflow resumes and reaches terminal success through fake steps. | EVD-1 |
...
| CTRL-6 | A milestone due point is reached without required evidence. | Stop dependent work until verifier approves, rejects, or records conditional approval. | Assigned work package owner | Milestone approval record. |
...
| VAL-1 | Test / Manual | Fake recipe critical path validates, starts, blocks, rejects stale decision, accepts matching decision, and completes. | Pre-merge for WP-1 | Workflow implementation agent | EVD-1 |
...
| Q-2 | Should queue listing stay in Temporal query state or use a derived local queue index? | Workflow implementation agent | Before MS-4 | No | Run 50-item seed measurement and choose the least stateful option that meets triage needs. |
...
Release gate:
Activation requires MS-5 approval, successful rollback drill, live opt-in smoke evidence or an explicit approved decision to defer live smoke, recipe-backed starts disabled-by-default or otherwise operator-controlled, and documented preservation of `agent.helloClaudex` or an approved equivalent.
