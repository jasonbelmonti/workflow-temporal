...
| Related tickets | BEL-910; BEL-1264 (Q-6); implementation-control Linear issue or project pending as an entry condition. |
...
Approved outcome:
Execute the composable agent workflow control-plane design from `SRC-1` by adding registry-backed `ProjectShapeContract` records, recipe records, optional external materializer records, derived project-shape transition diagnostics, bounded loop/guard semantics, an execution-profile model, a Temporal recipe interpreter, durable approval queue items, safe gate resume semantics, execution-profile enforcement, read-only triage/review behavior, run worktree containment, and operator evidence needed to activate the MVP.

Execution approach:
Use a risk-retirement and progressive-value sequence. First prove a fake recipe can validate, start, block on a durable queue item, and resume by queue item plus gate revision. Q-6 supersedes BEL-910/D-5 before WP-2: static registry workflow composition uses non-installable workflow package kinds, embedded `ProjectShapeContract` data, derived project-shape transition output, bounded loop transitions, and optional external materializers only for legacy or external transforms. Then harden registry contracts, Temporal state, execution-profile enforcement, worktree containment, project-state step contracts, queue triage, and live opt-in smoke evidence. Every implementation package shall have scoped validation, negative tests for the unsafe paths, and manual milestone approval before promotion.

Entry condition:
WP-1 execution shall not start until `SRC-1` is approved by Jason Belmonti, this execution spec is approved or conditionally approved for WP-1, Q-1 is resolved, a Linear execution-control issue or project exists for this implementation, and an independent capability-policy reviewer is assigned. BEL-910/D-5 remains historically resolved and Q-6 supersedes it for WP-2 registry composition; the registry dependency state remains a gate for WP-2 registry work, not a blocker for the fake proving slice.
...
- RISK-3: Registry recipes could degrade into prompt-shaped or project-shape-blind blobs instead of typed, validated execution contracts with explicit project facts, allowed reads, allowed writes, produced facts, gate outputs, artifact outputs, side effects, execution profiles, and loop semantics.
...
| SRC-8 | User project-shape pivot request on 2026-05-31 | Superseding planning authority for recipe composition. | Revises WP-2, WP-5, Q-5, MS-2, MS-4, VAL-19, and EVD-19 so recipes compose through durable project/workspace state contracts. BEL-910/D-5 is preserved as historical evidence and superseded by Q-6 rather than edited in place. |

In scope:
Implement the composable workflow MVP across `workflow-temporal` and `agent-config-registry`: `ProjectShapeContract` records, workflow recipe records, step definitions, optional external materializer records, derived project-shape transition diagnostics, bounded loop/guard transition fields, runtime `execution-profile` records, recipe validation, pinned recipe snapshots, a new recipe-backed Temporal workflow, fake and real project-state step runners, approval queue items, typed gate decision Updates, stale-decision rejection, worktree allocation, artifact references, execution-profile enforcement, read-only triage/review packets, visibility or projection listing, replay/versioning gates, five known happy-path project-state step contracts, tests, docs, evidence, and rollback controls.
...
| OBJ-2 | Registry content shall validate workflow recipes, step definitions, `ProjectShapeContract` fields, optional external materializers, and runtime execution profiles as typed records distinct from existing sync `profile` records. | Before real project-state step contracts are enabled. | EVD-2, EVD-4 |
...
| OBJ-6 | Mutating execution shall be isolated to one explicit run worktree and failed runs shall preserve artifacts for inspection and rollback. | Before mutating project-state step contracts are enabled. | EVD-7, EVD-10 |
...
| OBJ-8 | Registry workflow composition shall be governed by Q-6 before WP-2 starts, then proven through exported `ProjectShapeContract` records, optional external materializers, derived project-shape transition diagnostics, and bounded loop fixtures before MS-2 approval. | Q-6 before WP-2 starts; EVD-19 and EVD-20 before MS-2 approval. | Q-6, EVD-19, EVD-20 |
...
- DP-3: Q-6 supersedes BEL-910/D-5 before WP-2 by selecting project-shape primary recipe composition: additive non-installable package kinds for `workflow-recipe`, `step-definition`, optional external materializers, and runtime `execution-profile`; embedded `projectShapeContract` fields; guarded loop fields on recipe transitions; and derived project-shape transition catalog output. BEL-910 remains historical evidence and is not edited in place.
- DP-4: Approve the fake recipe-to-gate proving slice before real project-state step contracts or live providers are enabled.
...
- DP-7: Decide whether `consensus-review` is represented as one step with fanout internals or as a recipe subworkflow before WP-5 implements the real project-state step contract.
...
| CON-4 | Constraint | Runtime `execution-profile` records shall be distinct from existing registry sync `profile` records. | Registry implementation agent | Yes | Q-6 preserves the distinction from BEL-910/D-5; VAL-4 verifies it before recipes reference execution profiles. |
...
| CON-14 | Constraint | `agent-config-registry` shall remain a static configuration compiler for the MVP, not a runtime workflow-state service or always-on microservice. | Registry contract reviewer | Yes for WP-2 | Q-6 and VAL-19 record the selected static record/catalog strategy. |
| CON-15 | Constraint | Recipe transition legality shall be derived from `ProjectShapeContract` records and optional external materializer definitions, not manually authored as source truth or prompt guidance. | Registry implementation agent | Yes for WP-2 | VAL-19 verifies satisfied, missing-fact, forbidden-write, materializer-required, and composite-workflow-as-step fixtures. |
...
| ASM-1 | Assumption | Fake steps can prove recipe, queue, and resume semantics before real skills are represented as project-state steps. | Workflow implementation agent | No | VAL-1 proves the fake multi-run queue proof before WP-2 expands breadth. |
| ASM-2 | Assumption | The registry can add non-installable workflow package kinds for `workflow-recipe`, `step-definition`, optional external materializers, and runtime `execution-profile`; embedded `projectShapeContract` and guarded loop fields; and derived project-shape transition catalog metadata without breaking existing skill, agent-instruction, or sync-profile behavior. | Registry implementation agent | No | Q-6 supersedes BEL-910/D-5; VAL-4, VAL-16, VAL-19, and VAL-20 run compatibility tests across old and new records. |
...
| DEP-3 | Dependency | Registry contract, validation, and catalog work must be stable enough to extend before formal workflow record implementation starts. | Registry implementation agent | Yes before WP-2 | Q-6 records the selected project-shape workflow record strategy; DEP-3 records whether to wait for the active registry branch to stabilize or implement workflow records on a coordinated registry worktree. |
...
Execution shall retire the highest-risk claims before broad implementation: prove recipe/gate/resume semantics with fake steps first, harden registry project-shape contracts second, enforce capability and worktree containment before real skills, and add triage/live smoke only after queue and profile negative tests pass.

Validation cadence:
Every work package shall produce focused tests and an evidence note under `docs/evidence/composable-agent-workflow/`. Integration validation runs at each milestone. Negative tests for stale decisions, active heartbeat-backed cancellation, retry-policy and idempotency violations, read-only writes, disallowed tools, one-run-one-worktree containment, protected-path changes, project-shape transition diagnostics, invalid loop declarations, replay compatibility, and history/visibility growth are mandatory before live or mutating project-state step work.
...
| RISK-3 | Recipes could become prompt-shaped blobs. | Composition, validation, triage, and downstream step execution require typed project-state contracts. | Registry implementation agent | VAL-2 and VAL-4 reject prompt-only recipe and missing `ProjectShapeContract` records. | MS-2 |
| RISK-4 | Registry baseline is still in active MS-1 implementation. | Workflow-specific registry records may conflict with validation/catalog changes in progress. | Registry contract reviewer | Q-6, REV-2, DEP-3 readiness, and compatibility evidence from VAL-4. | MS-2 |
| RISK-10 | Project-shape transition validation could turn the registry into a runtime platform. | A registry microservice would add deployment, auth, persistence, and availability scope that the MVP explicitly avoids. | Registry contract reviewer | Q-6 records static-compiler strategy; VAL-19 proves transition output is derived and metadata-only. | MS-2 |
...
| Q-4 | Should the first `consensus-review` project-state step use the design-approved child-workflow fanout shape or the allowed single bounded step activity shape? | This changes step boundaries, workflow state shape, and review packet evidence while staying inside `SRC-1` D-1. | Jason Belmonti | Written decision before WP-5; VAL-8 verifies the step follows the approved shape. | MS-4 |
...
| --- | --- | --- | --- | --- | --- |
| SURF-1 | `agent-config-registry/src/agent-config-registry/contracts/**`, `schemas/**` | Contract / Schema | Registry implementation agent | Add non-installable workflow package kind contracts for `workflow-recipe`, `step-definition`, optional external materializers, and runtime `execution-profile`; embedded `projectShapeContract` fields; guarded loop transition fields; and project-shape transition metadata without weakening existing package kinds. | REV-2 |
| SURF-2 | `agent-config-registry/src/agent-config-registry/validation/**`, catalog/digest modules, tests | Code / Test | Registry implementation agent | Add validation and catalog behavior for workflow records, derived project-shape transition diagnostics, materializer-required transitions, forbidden write reporting, missing fact/view reporting, and bounded loop fixtures. | REV-2 |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| PKG-1 | Registry project-shape contracts | 2 | Define non-installable workflow recipe, step definition, optional external materializer, and runtime execution-profile package kinds; embedded `ProjectShapeContract` fields; guarded loop transition fields; and project-shape transition metadata contracts. | Recipes can be validated and composed through durable project state without prompt prose. | RISK-3, RISK-4, RISK-10, RISK-11 | Exported contract constants, types, schema records. | `npm test -- tests/contracts` in `agent-config-registry` | App-specific record semantics and no release compatibility process yet. |
| PKG-2 | Registry project-shape validation and catalog integration | 2 | Validate and catalog workflow records, references, profile compatibility, required project facts/views, allowed reads/writes, produced project facts, optional materializer requirements, project-shape transition diagnostics, and guarded loop semantics. | Registry can reject invalid recipes and explain project-state composition before workflow starts. | RISK-3, RISK-4, RISK-10, RISK-11 | Validation entrypoints, error codes, catalog record extensions, project-shape transition diagnostics. | `npm test -- tests/validation tests/catalog` in `agent-config-registry` | Depends on registry MS-1 validation/catalog baseline. |
...
| PKG-5 | Step activity runner and project-state step contracts | 2 | Execute one bounded typed step through fake or real agent runners that read and write declared project facts/views. | Real skills can run from declared project shape and produce declared project facts, artifacts, and gates. | RISK-3 | Step request/result contracts, project-state step registry, fake runner. | `npm test` in `workflow-temporal` | Skill-specific behavior and local provider assumptions. |
| PKG-6 | Execution-profile enforcement, worktree, and artifact containment | 2 | Apply capability policy, run worktree allocation, read-only bundles, and protected-path checks. | Mutating work is isolated and read-only work cannot write protected paths. | RISK-1 | Execution context, worktree manager, artifact references, enforcement layer. | `npm test` plus negative enforcement tests in `workflow-temporal` | Depends on local filesystem and tool availability. |
...
Mission:
Own registry-side typed contracts for workflow recipes, step definitions, `ProjectShapeContract` data, optional external materializers, guarded loop transitions, project-shape transition metadata, and runtime execution profiles.

Value / risk trace:
- Observable value enabled: registry content can express and explain composable workflows without hard-coded Temporal step order or prompt-inferred project state.
...
- Concepts: package kinds or record types, schema versions, contract field names, exported `ProjectShapeContract` data, optional external materializer records, guarded loop fields, project-shape transition metadata, and compatibility between recipes and execution profiles.
...
Public interface:
- Exported types: recipe, step definition, `ProjectShapeContract`, optional external materializer, project-shape transition diagnostic, loop transition, execution profile, gate policy, retry policy, capability policy.
...
- Required coordination before editing: package kind names, schema version fields, project-shape diagnostic format, guarded loop fields, or existing `profile` semantics.
...
Mission:
Own registry-side validation and catalog behavior for workflow records, derived project-shape transition diagnostics, materializer-required transitions, forbidden writes, missing facts/views, and guarded loop semantics.

Value / risk trace:
- Observable value enabled: invalid workflow recipes, profile references, missing project facts/views, forbidden writes, missing external materializers, and unbounded loops fail before a workflow starts.
- Risk retired: RISK-3, RISK-4, RISK-10, RISK-11.
- Validation evidence: VAL-2, VAL-4, VAL-16, VAL-19, VAL-20, EVD-2, EVD-4, EVD-16, EVD-19, EVD-20.
...
- Concepts: reference validation, missing-contract errors, prompt-only rejection, project-shape transition classification, materializer requirement diagnostics, loop validation, catalog metadata.
...
Public interface:
- Exported types: validation result, project-shape transition diagnostic, and error codes for workflow records.
...
- Required coordination before editing: validation result shape, public error codes, project-shape transition metadata, generated catalog fields.
...
- May call: registry validation and fetch routines from side-effectful contexts.
...
Mission:
Own fake and real bounded project-state step execution and typed project-shape mapping.

Value / risk trace:
- Observable value enabled: recipe steps can execute fake and real skill-backed work from declared project facts, views, artifacts, and gates.
- Risk retired: RISK-3.
- Validation evidence: VAL-1, VAL-8, EVD-1, EVD-8.
- Blocking unknowns: Q-1.

Owns:
- Files/directories: `workflow-temporal/src/activities/**`, step runner modules, project-state step tests.
- Concepts: step request, step response, project facts/views, allowed reads, allowed writes, produced facts, gate request, artifact refs, provider session hints.
...
Public interface:
- Exported types: step activity request/result and project-state step contract.
- Exported functions/classes/components: fake runner and project-state step registry.
- Events/messages/contracts: gate request output contract.
- CLI/API surface: none.

Allowed dependencies:
- May import: activity context, Claudex turn runner, approved skill runners, execution context from PKG-6.
...
Agent ownership boundary:
- Agent editable paths: step activity, project-state step, and runner modules plus focused tests.
...
Promotion blockers:
Project-state step set is MVP-specific and tied to locally installed skills.
...
Owns:
- Files/directories: worktree manager, artifact store, enforcement layer, protected-path policy modules, negative tests.
...
- Exported functions/classes/components: enforcement module, worktree manager, artifact writer.
...
Forbidden dependencies:
- Must not import: project-state step internals or worktree mutators.
...
- A real skill project-state step requires editing registry schema fields after MS-2 approval.
...
Planning strategy:
Risk retirement with progressive value. WP-1 proves the critical path with fake steps. WP-2 and WP-3 harden source authority and workflow state. WP-4 blocks unsafe capability drift before real skills. WP-5 and WP-6 add real project-state step and operator value. WP-7 performs activation, rollback, and handoff.
...
| WP-2 | Harden Q-6 registry project-shape contracts and transition validation. | Registry implementation agent | PKG-1, PKG-2 | `agent-config-registry` contracts, schemas, validation/catalog tests | `workflow-temporal` design and resolver tests | WP-1 fixture, registry MS-1 baseline, Q-6 decision | Validated `ProjectShapeContract`, recipe, step definition, optional materializer, loop transition, execution-profile records, derived project-shape transition diagnostics, and catalog metadata | DEP-3, Q-6 | Registry rejects invalid or prompt-only recipes, proves required facts are satisfied, produced facts are declared, protected writes are denied, external materializers are explicit, unbounded loops are rejected, and pinned recipe compatibility is preserved across schema/profile evolution | RISK-3, RISK-4, RISK-10, RISK-11 | MS-2 | VAL-2, VAL-4, VAL-16, VAL-19, VAL-20 | Existing registry tests remain green; new workflow validation tests pass; project-shape fixtures classify satisfied, missing-fact, forbidden-write, materializer-required, and composite-workflow-as-step transitions; loop fixtures accept bounded loops and reject unbounded loops; older pinned recipe snapshots remain executable after newer compatible schema or execution-profile records are published; reviewer approves transition validation. |
...
| WP-4 | Implement execution-profile enforcement and worktree/artifact containment. | Capability-policy implementation agent | PKG-6 | Enforcement, worktree, artifact modules and tests | Workflow queue contracts, registry profile contracts | WP-2 execution-profile schema; WP-3 state | Fail-closed execution context, one-run-one-worktree allocation, read-only bundle, failed-worktree preservation, protected-path diff checks | MS-2 | Read-only and mutating execution profiles have enforceable runtime and worktree boundaries before real project-state steps run | RISK-1 | MS-3 | VAL-7, VAL-10 | Negative and containment tests prove protected writes, disallowed tools, disallowed skills, worker-agent/delegation denial, write-capable tool denial, no mutable worktree path for read-only profiles, approval-Update authority denial, one-run-one-worktree allocation, and failed-worktree preservation before WP-5 can proceed. |
| WP-5 | Implement happy-path project-state step contracts and approved consensus review shape. | Step contract implementation agent | PKG-5, PKG-6 | Step contract modules, fixtures, project-state step tests | Skill source docs, registry contracts, workflow state | Approved profile enforcement, Q-4 consensus-review shape decision, and Q-5 project-shape fact/view vocabulary | Project-state step contracts for `linear-next-task`, `handoff-prompt`, `code-simplifier`, `organize-code-boundaries`, and `consensus-review`; one validated five-step happy-path recipe; fake fallbacks; optional materializer only where a skill cannot directly read or write the project shape | MS-3, Q-4, Q-5 | Recipe can represent and execute the intended workflow path through durable project state | RISK-3 | MS-4 | VAL-8 | Project-state step tests show required facts/views, allowed reads/writes, produced facts, artifact outputs, gate outputs, side-effect class, and execution-profile constraints; registry validation accepts one full five-step recipe ordered as `linear-next-task` -> `handoff-prompt` -> `code-simplifier` -> `organize-code-boundaries` -> `consensus-review` without hard-coded workflow-code step order or prompt-only implicit state; `consensus-review` follows the approved Q-4 shape under a read-only profile. |
...
2. Execute WP-1 as the first proving slice. Any registry-adjacent work in WP-1 is workflow-local fixture-only, shall not edit `agent-config-registry`, and shall not override the Q-6 package-kind, schema, catalog, project-shape transition, optional materializer, or loop strategy.
3. Execute WP-2 and WP-3 after MS-1; Q-6 must be recorded before WP-2 starts, DEP-3 must still be resolved before WP-2 starts, and WP-2 must stabilize registry contracts before WP-3 locks workflow snapshot shapes.
4. Execute WP-4 before any real project-state step receives write-capable tools.
...
Parallel work is prohibited before WP-1 approval. After MS-2, registry validation work and workflow queue work may proceed in parallel only when contract files are not shared. After MS-3, project-state step contracts and triage may proceed in parallel if neither changes queue item or execution-profile public contracts.
...
| MS-2 | Approve Q-6 registry project-shape contracts and transition model. | OBJ-2, OBJ-8, WP-2, PKG-1, PKG-2, SURF-1, SURF-2 | Before WP-4 and before real project-state steps reference execution profiles | Jason Belmonti | Q-6 decision recorded; BEL-910/D-5 referenced as superseded historical evidence; named `REV-2` reviewer assignment recorded in EVD-0; VAL-2, VAL-4, VAL-16, VAL-19, VAL-20, EVD-2, EVD-4, EVD-16, EVD-19, EVD-20 | REV-2 | EVD-2, EVD-4, EVD-16, EVD-19, EVD-20, Q-6 decision record | Approve / Reject / Conditional approval | Stop registry integration; revise package-kind/schema strategy, static compiler boundary, project-shape transition posture, optional materializer semantics, loop semantics, or pinned snapshot compatibility. |
| MS-3 | Approve queue safety and capability containment. | OBJ-3, OBJ-4, OBJ-5, OBJ-6, WP-3, WP-4, PKG-4, PKG-6 | Before WP-5 real project-state steps or WP-6 triage | Jason Belmonti | Named `REV-3`, `REV-4`, and `REV-5` reviewer assignments recorded in EVD-0; VAL-3, VAL-5, VAL-6, VAL-7, VAL-10, VAL-13, VAL-14, VAL-15, VAL-17, EVD-3, EVD-5, EVD-6, EVD-7, EVD-10, EVD-13, EVD-14, EVD-15, EVD-17 | REV-3, REV-4, REV-5 | EVD-3, EVD-5, EVD-6, EVD-7, EVD-10, EVD-13, EVD-14, EVD-15, EVD-17 | Approve / Reject / Conditional approval | Disable recipe-backed starts; keep fake slice only until defects are resolved. |
| MS-4 | Approve project-state step contracts and triage usability. | OBJ-1, OBJ-5, OBJ-7, WP-5, WP-6, PKG-5, PKG-7 | Before live opt-in smoke or final readiness | Jason Belmonti | Q-4, Q-5, VAL-8, VAL-9, VAL-18, EVD-8, EVD-9, EVD-18 | REV-1A, REV-4, REV-5 | EVD-8, EVD-9, EVD-18, Q-4 decision record, Q-5 project-shape vocabulary decision record | Approve / Reject / Conditional approval | Remove real project-state steps from activation path; continue with fake recipe evidence only. |
...
| MV-4 | MS-2 | Inspect registry contract, validation, project-shape transition, and loop evidence. | Runtime `execution-profile` is distinct from sync `profile`; prompt-only and missing-reference recipes fail validation; project-shape transition diagnostics classify satisfied, missing-fact, forbidden-write, materializer-required, and composite-workflow-as-step transitions; bounded loop fixtures pass; invalid loop declarations fail; older pinned recipe snapshots remain executable after newer compatible schema or execution-profile records are published. | EVD-2, EVD-4, EVD-16, EVD-19, EVD-20 |
...
| MV-7A | MS-4 | Inspect or run the five happy-path recipe and project-state step evidence. | One validated recipe represents `linear-next-task` -> `handoff-prompt` -> `code-simplifier` -> `organize-code-boundaries` -> `consensus-review` without workflow-code step ordering; each step exposes required facts/views, allowed reads/writes, produced facts, artifact outputs, gate outputs, side-effect class, and execution-profile constraints; project-shape transition output explains required facts, produced facts, protected write posture, side effects, and profile constraints; `consensus-review` follows the Q-4 approved shape and is bound to a read-only execution profile. | EVD-8, EVD-9, EVD-19 |
...
| CTRL-5 | Registry workflow records conflict with existing `profile` package semantics. | Pause WP-2 and reconcile the implementation against Q-6 with the registry reviewer and Jason; update Q-6 or record an approved deviation if the selected strategy must change. | Registry implementation agent | EVD-4 decision record. |
...
| CTRL-11 | WP-2 implementation deviates from Q-6 for `ProjectShapeContract` fields, optional external materializer semantics, derived project-shape transition output, loop semantics, pinned snapshot compatibility, or the no-microservice boundary. | Stop WP-2 and keep divergent registry workflow records unimplemented until Q-6 is updated or an approved deviation is recorded. | Registry contract reviewer | Q-6 decision record, EVD-19, EVD-20. |
...
| --- | --- | --- | --- | --- |
| Add registry `workflow-recipe` records. | New non-installable package kind for ordered, branched, or guarded-loop workflow steps with exported composite `ProjectShapeContract`. | Existing registry package kinds must continue to validate and catalog unchanged; older pinned recipe snapshots must remain executable after newer compatible schema or execution-profile records are published. | Reversible by removing records and disabling recipe-backed starts. | VAL-2, VAL-4, VAL-16 |
| Add registry `step-definition` records. | New non-installable package kind defining required project facts/views, allowed reads, allowed writes, produced project facts, artifact outputs, gate outputs, side-effect class, and execution-profile constraints for recipe steps. | Existing skills are referenced by project-state step contracts; skill package content does not need immediate rewrite. | Reversible by removing step records and project-state step contracts. | VAL-2, VAL-8 |
| Add embedded `projectShapeContract` schema fields. | Defines the exported composition interface shared by primitive steps and composite workflows without a standalone source-record kind for MVP. | Existing package kinds must ignore or reject unsupported project-shape metadata by schema version; composite workflows can be reused as steps only through this public contract. | Reversible by removing fields and disabling recipe-backed starts. | VAL-19 |
| Add optional external materializer records. | New non-installable package kind defining explicit legacy or external transforms only when a step cannot directly read from or write to the project shape. | Existing recipes remain valid; materializer-required transitions are diagnostics until a recipe references a materializer. | Reversible by removing materializer records and recipes that depend on them. | VAL-19 |
| Add derived project-shape transition catalog metadata. | Explains satisfied, missing-fact, forbidden-write, materializer-required, and composite-workflow-as-step composition relationships. | Transition output is metadata-only and derived from source records, not source truth. | Reversible by regenerating catalog without transition metadata and disabling transition-dependent starts. | VAL-19 |
...
| VAL-2 | Test / Review | Registry rejects missing `ProjectShapeContract` records, missing execution profiles, prompt-only outputs, unsupported schema versions, invalid references, undeclared produced facts, and protected write attempts. | Pre-merge for WP-2 | Registry implementation agent | EVD-2 |
...
| VAL-8 | Test | One validated recipe represents `linear-next-task` -> `handoff-prompt` -> `code-simplifier` -> `organize-code-boundaries` -> `consensus-review` without hard-coded workflow-code step order; the five happy-path project-state step contracts declare required facts/views, allowed reads/writes, produced facts, artifact outputs, gate outputs, side-effect class, execution-profile constraints, and the Q-4 approved `consensus-review` step or subworkflow shape. | Pre-release | Step contract implementation agent | EVD-8 |
...
| VAL-10 | Test / Manual / Inspection | Worktree allocation is one run to one mutable worktree, failed worktrees are preserved, and cleanup is explicit. | Pre-merge for WP-4; required before WP-5 mutating project-state steps | Workflow operator and capability-policy reviewer | EVD-10 |
...
| VAL-13 | Test / Fault injection | Provider-session loss resumes from workflow-owned objective, summaries, human inputs, project facts/views, and artifact refs rather than provider-local session state; a new provider session hint is recorded when available. | Pre-merge for WP-3; repeat before live smoke if step behavior changes | Workflow implementation agent | EVD-13 |
...
| REV-1A | Jason Belmonti | Q-4 `consensus-review` execution-shape decision and Q-5 project-shape fact/view vocabulary decision before WP-5 and MS-4 project-state step approval. | Yes before WP-5 | Q-4 decision record and Q-5 project-shape vocabulary decision record before WP-5 starts. |
| REV-2 | Registry contract reviewer | Q-6 decision, registry contracts, schema compatibility, validation, catalog, digest, `ProjectShapeContract` fields, optional external materializers, derived project-shape transition metadata, guarded loop fields, runtime `execution-profile` distinction, and older pinned recipe compatibility across compatible schema/profile evolution. | Yes | MS-2 approval with EVD-2, EVD-4, EVD-16, EVD-19, and EVD-20. |
...
| REL-3 | Enable real project-state steps only after MS-4 approval, including Q-4/Q-5 resolution and VAL-8 project-state evidence. | After WP-5 / MS-4 | Capability-policy reviewer | Read-only protected-path negative test, worktree containment proof, approved consensus-review shape, or project-shape contract evidence fails. | EVD-7, EVD-8, EVD-10 |
...
| OBS-10 | Registry project-shape transition diagnostics | Explains satisfied, missing-fact, forbidden-write, materializer-required, and composite-workflow-as-step transitions derived from `ProjectShapeContract` records and optional materializer records. | Registry contract reviewer / operator | Block WP-2 or recipe publication until diagnostics match Q-6 and VAL-19. |
...
| RISK-3 | Recipes become prompt-shaped blobs. | Medium | Medium | Registry implementation agent | Require typed `ProjectShapeContract` definitions, required facts/views, allowed reads/writes, produced facts, artifacts, and gate contracts; reject prompt-only records. | VAL-2, VAL-4, VAL-8 |
...
| RISK-10 | Project-shape transition validation turns registry into a runtime platform. | Medium | Medium | Registry contract reviewer | Keep registry as static compiler, derive metadata-only transition output, and defer any service until explicit service trigger conditions exist. | VAL-19, REV-2 |
...
`SRC-1` Q-1 maps to this spec's Q-2 queue storage decision. `SRC-1` D-1 constrains this spec's Q-4 consensus-review execution-shape decision. Q-6 supersedes the prior Q-3/BEL-910 registry composition strategy for WP-2. This spec's Q-1 and Q-5 are execution-specific gates added during planning.
...
| Q-4 | Should the first `consensus-review` project-state step use the design-approved child-workflow fanout shape or the allowed single bounded step activity shape? | Jason Belmonti | Before WP-5 starts | Yes for WP-5 | Record the design-owner decision and validate the selected shape in VAL-8 before implementing the real project-state step. |
| Q-5 | What initial `ProjectShapeContract` fact/view vocabulary is required for the five happy-path skills? | Step contract implementation agent drafts; Jason Belmonti approves with workflow and registry reviewer input | Before WP-5 starts | Yes for WP-5 | Draft a fact/view vocabulary and step contract table, record a Q-5 project-shape vocabulary decision artifact, review it against workflow and registry contracts, and validate with fake fixtures before real skills. |
...
| SRC-8 / Q-6 / Project-shape pivot | SURF-1, SURF-2, SURF-7, SURF-9 | PKG-1, PKG-2 | WP-2 | MS-2 | CTRL-5, CTRL-11 | VAL-2, VAL-4, VAL-16, VAL-19, VAL-20 | REV-2 | OBS-10, OBS-11 | Q-6 decision record, BEL-910 supersession note, EVD-2, EVD-4, EVD-16, EVD-19, EVD-20 |
...
| OBJ-8 / Registry project-shape composition and bounded loops | SURF-1, SURF-2, SURF-7 | PKG-1, PKG-2 | WP-2 | MS-2 | CTRL-5, CTRL-11 | VAL-19, VAL-20 | REV-2 | OBS-10, OBS-11 | EVD-19, EVD-20 |
...
| Q-6 / Project-shape registry record strategy | SURF-1, SURF-2, SURF-7, SURF-9 | PKG-1, PKG-2 | WP-2 | MS-2 | CTRL-5, CTRL-11 | VAL-2, VAL-4, VAL-16, VAL-19, VAL-20 | REV-2 | OBS-10, OBS-11 | Q-6 decision record; BEL-910 approval of D-5 preserved as superseded evidence; EVD-2, EVD-4, EVD-16, EVD-19, EVD-20 |
...
| Q-5 / Project-shape fact/view vocabulary | SURF-4, SURF-6, SURF-8 | PKG-5, PKG-6 | WP-5 | MS-4 | CTRL-1, CTRL-3 | VAL-8 | REV-1A, REV-4 | REL-3, OBS-3 | Q-5 project-shape vocabulary decision record, EVD-8 |
...
Entry gate:
Not satisfied. Required before WP-1 starts: Jason approves `SRC-1`; Jason approves this execution spec or conditionally approves it for WP-1; Q-1 is resolved; a Linear execution-control issue or project exists with a `task-definition` compliant work-item body and traceability; and an independent capability-policy reviewer is assigned. BEL-910/D-5 is preserved as superseded historical evidence; Q-6 and DEP-3 are required before WP-2 starts, not before the fake proving slice.
