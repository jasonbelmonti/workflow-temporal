...
import {
  ...
  readClaudexTurnModel,
  resolveClaudexTurnTimeoutMs
...
export const defaultLiveClaudexTurnTimeoutMs = 60_000;
...
export { claudexTurnModelEnvName, claudexTurnTimeoutEnvName };
...
export function buildLiveHelloClaudexSmokeConfig({
  ...
  const turnTimeoutMs = resolveClaudexTurnTimeoutMs(env, defaultLiveClaudexTurnTimeoutMs);
  const turnModel = readClaudexTurnModel(env);
